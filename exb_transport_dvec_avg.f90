...
!   average vppn to center of cell face
                ...
                vexbp(i,j,k,ni) = vexbp_phi(i,j,k) / factor0 - &
                 ...
                 (-1.0) *  uu * factor1 * &
                 0.25 * ( dvec(i,j,k,2,1) + dvec(i,j-1,k,2,1)  + &
                          dvec(i,j,k+1,2,1) + dvec(i,j-1,k+1,2,1) ) + &
                 (-1.0) *  vv * factor2 * &
                 0.25 * ( dvec(i,j,k,2,2) + dvec(i,j-1,k,2,2)  + &
                          dvec(i,j,k+1,2,2) + dvec(i,j-1,k+1,2,2) ) + &
...
!   uu/vv are u and v at cell interface
              ...
              vexbh(i,j,k,ni) = vexbh_phi(i,j,k) / factor0 + &
                  ...
                  vv * factor1 *  &
                 0.5 * ( dvec(i,j,k,1,2) + dvec(i,j,k-1,1,2) )  + &
                  uu * factor2 * &
                 0.5 * ( dvec(i,j,k,1,1) + dvec(i,j,k-1,1,1) )
                if ( baltp(i,j,k) > alt_crit_high ) then
                    arg0 = ( abs(alt_crit_high - baltp(i,j,k)) ) / dela_high
                    fac = exp(-arg0*arg0)
...
!     assume longitude grid is uniform
              (2. * vexbp_phi(i,j,k-2) - vexbp_phi(i,j,k-1)) * factor1 - &
                  vv * factor1 * dvec(i,j,k-1,1,2) + &
                  uu * factor2 * dvec(i,j,k-1,1,1)  
                if ( baltp(i,j,k) > alt_crit_high ) then
                    arg0 = ( abs(alt_crit_high - baltp(i,j,k)) ) / dela_high
                    fac = exp(-arg0*arg0)
                    vexbh(i,j,k,ni) = vexbh(i,j,k,ni) * fac
