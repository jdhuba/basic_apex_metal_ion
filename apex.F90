...
!   if (qdlon > 30. .and. qdlon < 60. .and. abs(qdlat) < 1.) write(6,"('intrp: iter=',i3,' fx,fy,fz,x0,y0,z0,dist2 = ',7g10.4)")iter,fx,fy,fz,x0,y0,z0,dist2

    if (dist2 <= 0.1*precise) then
      ier = 0
      gdlat = ylat
      gdlon = ylon
!     if (qdlon > 40. .and. qdlon < 70. .and. abs(qdlat) < 1.) write(6,"('apex_q2g: iter=',i3,' final ylat,ylon,dist2 = ',3f10.4)")iter,ylat,ylon,dist2
      return
    endif
!
...
!
  ...
  j0 = 0
  do j=1,nlon-1
    if (glonloc >= gplon(j).and.glonloc <= gplon(j+1)) then
      j0 = j
      dlon = gplon(j+1)-gplon(j)
      yj = (glonloc - gplon(j)) / dlon
      exit 
    endif
  enddo
  if (j0==0) then
...
!     alt,gpalt
    write(6,"('>>> intrp: could not bracket alt,glat,glon=',3f10.2,' in gpalt=',/,(6f12.2))") &
      alt,glat,glon,gpalt
!   ier = 1
!***    return ***
  endif
...
! Local:
  ...
  integer,parameter :: &
    ...
    ncn2=6     ! number of coefficients dimensioned n2
