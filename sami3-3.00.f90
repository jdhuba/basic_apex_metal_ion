...
! Calculate APEX magnetic field using apex_suns  4/5/2013 (GL)
      ...
      write(6,"('initial call apxparm: year = ',f7.0)") year
      call apxparm(year)
...
!      prod(i,ptnop) = 0.
    ...
    !     loss term for mg+ and fe+ (from mcneil et al. (2001) - fig 2)
        ...
        if ( alts(i,nfl,nll) < 100. ) then
            xloss_100  = 1.e-13
            delx_loss  = .6
            xloss_rate = xloss_100 * exp(-(alts(i,nfl,nll)-100.)/delx_loss)
...
!!$        if ( taskid == 1 .and. nfl == 43 .and. nll == nl/2 ) then
!!$          do ii = 1,nz
!!$            print *,'loss = ',ii,alts(ii,nfl,nll),loss(ii,ptmgp),loss(ii,ptop)
!!$          enddo
!!$        endif
